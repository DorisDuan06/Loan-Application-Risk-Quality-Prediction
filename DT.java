...
public class DT {
    ...
    public static void main(String[] args) {
        if (args.length < 3) {
            System.out.println("usage: java <modeFlag: 0, 1, 2, 3> <trainFilename> <testFilename>");
            System.exit(-1);
        }
    ...
    /**
     * Converts from text file format to DataSet format. All data files (training, tuning, test)
     * will contain a list of classes and attribute values, followed by the actual data. Attributes
     * and classes are always discretely valued. A line that begins with a double slash // is a
     * comment and should be ignored. In other lines, elements will be comma-separated. For a line
     * beginning with %%, it contains two possible classes. Each line that begins with ## specifies
     * the name and all possible discrete values of one attribute. The order of successive attributes
     * is important as this is the same order used in each of the examples in the file.
     */
